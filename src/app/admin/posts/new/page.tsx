...
export default function NewPostPage() {
  ...
  // ── raw content (sent to API on save) ─────────────────────────────────────
  ...
  function applyParsedDraft(file: File) {
    ...
    reader.onload = (e) => {
      ...
      // Populate form — always overwrite with parsed values
  ...
  function handleFileInput(file: File) {
    ...
    const alreadyHasContent =
      title.trim() || contentRaw.trim()

    if (alreadyHasContent && mdFile) {
  ...
  async function handleSave(publishNow: boolean) {
    ...
    try {
      const body = {
        ...
        contentRaw,          // server generates contentHtml from this
        ...
        {parseState === 'idle' || !mdFile ? (
          ...
          <div className={cn(
            ...
            <div className="flex-1 min-w-0">
              <p className="text-sm font-medium text-[#181d12] dark:text-[#f7fce9] truncate">
                {mdFile.name}
              </p>
              {parseState === 'parsing' && (
                <p className="text-xs text-[#767870] dark:text-[#464841] mt-0.5">Parsing…</p>
              )}
              {parseState === 'ok' && (
            ...
            <div className="flex items-center gap-2 shrink-0">
              <button
                onClick={() => fileInputRef.current?.click()}
                className="text-xs text-[#5b6300] dark:text-[#c2cf47] hover:underline"
              >
                Replace
              </button>
              <button
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <Field label="Category *">
              <select
                value={category}
                onChange={(e) => setCategory(e.target.value as 'stories' | 'tech' | 'insights')}
                className={inputClass}
              >
          ...
          <label className="flex items-center gap-3 cursor-pointer">
            <input
              ...
              className="w-4 h-4 rounded accent-[#5b6300]"
            />
            <span className="text-sm text-[#464841] dark:text-[#c6c7be]">
        ...
        {coverImageUrl ? (
          <div className="flex items-center gap-3 p-3 bg-[#f7fce9] dark:bg-[#2d3226]/40 rounded-[0.75rem]">
            <img
              src={coverImageUrl}
              alt="Cover preview"
              className="w-16 h-16 object-cover rounded-[0.5rem] shrink-0"
            />
            <div className="flex-1 min-w-0">
              <p className="text-xs text-[#181d12] dark:text-[#f7fce9] truncate font-mono">
                {coverImageUrl}
              </p>
            </div>
            <button
              onClick={() => setCoverImageUrl('')}
              className="text-[#767870] hover:text-red-500 transition-colors"
            >
              <X className="w-4 h-4" />
            </button>
          </div>
