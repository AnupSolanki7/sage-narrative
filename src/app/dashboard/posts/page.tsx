...
import UserPostActions from '@/components/dashboard/UserPostActions'
...
export default async function DashboardPostsPage() {
  ...
  try {
    posts = await getUserPosts(session.userId!)
  } catch {}
            ...
            <tbody className="divide-y divide-[#e0e5d2] dark:divide-[#2d3226]">
              {posts.map((post) => (
                ...
                >
                  ...
                  <td className="px-6 py-4">
                    <UserPostActions postId={post._id} status={post.status} slug={post.slug} />
                  </td>
                </tr>
              ))}
            </tbody>
