...
import { getUserPostStats, getUserPosts } from '@/lib/db/posts'
...
export default async function DashboardPage() {
  ...
  try {
    ;[stats, recentPosts] = await Promise.all([
      getUserPostStats(session.userId!),
    ...
    <div className="max-w-5xl mx-auto">
      <div className="flex items-center justify-between mb-8">
        <div>
          ...
          <p className="text-sm text-[#767870] dark:text-[#464841] mt-1">
            Welcome back. Here&apos;s your writing overview.
          </p>
        </div>
        <Link
      ...
      <div className="bg-white dark:bg-[#1c2217] rounded-[1.5rem] border border-[#e0e5d2] dark:border-[#2d3226] overflow-hidden">
        <div className="flex items-center justify-between px-6 py-4 border-b border-[#e0e5d2] dark:border-[#2d3226]">
          ...
          <Link href="/dashboard/posts" className="text-xs text-[#5b6300] dark:text-[#c2cf47] font-medium hover:underline">
            View all
          </Link>
        </div>
        ...
        {recentPosts.length === 0 ? (
          <div className="px-6 py-12 text-center">
            <p className="font-serif italic text-[#767870] dark:text-[#464841]">
              You haven&apos;t written anything yet.
            </p>
            <Link
              href="/dashboard/posts/new"
              className="mt-3 inline-block text-sm text-[#5b6300] dark:text-[#c2cf47] hover:underline"
            >
          ...
          <ul className="divide-y divide-[#e0e5d2] dark:divide-[#2d3226]">
            {recentPosts.map((post) => (
              <li
                key={post._id}
                className="flex items-center gap-4 px-6 py-4 hover:bg-[#f7fce9] dark:hover:bg-[#2d3226]/30 transition-colors"
              >
