...
import { Upload, Image, Loader2, Eye, EyeOff, Trash2, X, ExternalLink } from 'lucide-react'
...
export default function EditPostForm({ post }: EditPostFormProps) {
  ...
  async function handleMdReplace(file: File) {
    ...
    try {
      ...
      // Send contentRaw only — the PUT route regenerates contentHtml server-side
      // so frontmatter never reaches the HTML renderer.
  ...
  async function handlePublishToggle() {
    ...
    try {
      await fetch(`/api/posts/${post._id}/publish`, { method: 'POST' })
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white dark:bg-[#1c2217] rounded-[1.5rem] border border-[#e0e5d2] dark:border-[#2d3226] p-6">
        <h2 className="font-semibold text-[#181d12] dark:text-[#f7fce9] mb-5 flex items-center gap-2">
          <Image className="w-4 h-4 text-[#5b6300] dark:text-[#c2cf47]" />
          Cover Image
        </h2>
