...
export default function UserPostActions({ postId, status, slug }: UserPostActionsProps) {
  ...
  async function handlePublishToggle() {
    setLoading('publish')
    try {
      await fetch(`/api/user/posts/${postId}/publish`, { method: 'POST' })
      router.refresh()
    } finally {
      setLoading(null)
    }
  ...
  async function handleDelete() {
    ...
    setLoading('delete')
    try {
      await fetch(`/api/user/posts/${postId}`, { method: 'DELETE' })
      router.refresh()
    } finally {
      setLoading(null)
    }
    ...
    <div className="flex items-center justify-end gap-1">
      {status === 'published' && (
        <Link
          href={`/blog/${slug}`}
          target="_blank"
          className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium text-[#464841] dark:text-[#c6c7be] hover:bg-[#f1f6e3] dark:hover:bg-[#2d3226]/50 transition-colors"
